...
BUILD_RFS   EQU     1                   ; Build BASIC SA-5510 for use under RFS with SD Card access.
...
BOOTNAME:   IF BUILD_ORIG = 1
            ...
            ENDIF
            IF BUILD_RFS = 1
              DB    "BASIC SA-5510RFS", 0Dh                              ; Title/Name (17 bytes).
            ENDIF
...
TAPECOPY:   IF BUILD_ORIG = 1
              JP    TAPECOPYPRG
            ENDIF
            IF BUILD_RFS = 1                                             ; Under RFS build the tape copy code isnt needed so place NOP's where the call resides.
              NOP
              NOP
              NOP
            ENDIF
...
COLDSTRT:   LD      HL,048FFH           ; Scan for memory top.
            IF BUILD_ORIG = 1
              LD    D,0D0H              ; Full memory for original build
            ENDIF
            IF BUILD_RFS = 1
              LD    D,0CEH              ; RFS uses the top 512byte sector for SD card operations so need to limit memory.
            ENDIF
L1205:      INC     HL
            LD      A,H
            CP      D
...
L4223:        DB    001H,"BASIC SA-5510",00DH
            ...
            ; Define extensions under RFS. Each is in a seperate block due to Glass Z80 way of not evaluating labels inside IF statements when false.
            ;
RFSRDI:     IF BUILD_RFS = 1
              ; RFS extensions, call the RFS API to execute extended code.
              LD    DE,CMTFNAME
              JP    CMT_RDINF
            ENDIF
CMDDIR:     IF BUILD_RFS = 1
              CALL  L193B               ; Skip space to CR, ':' or char.
              LD    (05051H),HL
